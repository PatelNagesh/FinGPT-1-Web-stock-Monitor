...
import { getQuote, getCompanyNews, getMarketNews, getCandles, MarketQuote } from './services/marketService';
...
export default function App() {
  ...
  const [marketData, setMarketData] = useState<{ quote: MarketQuote | null, news: any[], candles: any[] }>({
    quote: null,
    news: [],
    candles: []
  });
  ...
  const fetchData = useCallback(async (symbols: string[], range: TimeRange) => {
    ...
    try {
      ...
      // Fetch primary symbol data (quote and news)
      ...
      setMarketData({ quote, news, candles: formattedCandles });
    } catch (e: any) {
      console.error(e);
      let msg = e.message || 'Failed to fetch market data.';
      if (msg.includes('FINNHUB_API_KEY')) {
        msg = 'Connection Error: Market Data Secret (FINNHUB_API_KEY) is not configured in the AI Studio Secrets panel.';
      }
      setError(msg);
    } finally {
      setLoading(false);
          ...
          <div className="flex-1 overflow-auto p-8 space-y-6">
            <header className="flex items-end justify-between mb-2">
              <div className="space-y-1">
                ...
                <p className="text-sm text-slate-500 font-medium">AI-driven predictive modeling for high-performance trading.</p>
              </div>
